...
interface Punto extends SbBlokData {
    _uid: string
    component: string
    posicion: string // ej: "top-[38%] left-[45%]"
    nombre: string
    precio: string
    id_producto: string
    color_punto: string
...
interface Producto extends SbBlokData {
    _uid: string
    component: string
...
const PopularProduct = ({ blok }: PopularProductProps) => {
    const router = useRouter()
    ...
    const columnas = blok.columnas === '2' ? 'sm:grid-cols-2' : 'sm:grid-cols-3'

    return (
        <div className="popular-product md:py-6 py-6" {...storyblokEditable(blok)}>
            <div className={`list-product grid ${columnas}`}>
                {blok.productos
                    ...
                    .map((producto) => (
                        <div
                            ...
                            {...storyblokEditable(producto)}
                        >
                            {producto.imagen?.filename && (
                                <Image
                                    src={producto.imagen.filename}
                                    width={1000}
                                    height={1000}
                                    alt={producto.imagen.filename || 'product'}
                                    priority
                            ...
                            {producto.puntos
                                ?.filter((dot) => dot.status)
                                .map((dot) => (

                                    <div key={dot._uid} >
                                        <div className="dots absolute top-[34%] left-[35%] cursor-pointer"{...storyblokEditable(dot)}>
                                            <div className="top-dot w-8 h-8 rounded-full bg-outline flex items-center justify-center"
                                                style={{ backgroundColor: dot.color_punto || '#ccc' }}
                                            >
                                                <span className="bg-white w-3 h-3 rounded-full duration-300"></span>
                                            </div>
                                            <div className="product-infor bg-white rounded-2xl p-4" onClick={() => handleDetailProduct(dot.id_producto)}>
                                                <div className="text-title name">{dot.nombre}</div>
                                                <div className="price text-center">{dot.precio}</div>
                                                <div
                                                    className="text-center underline mt-1 text-button-uppercase duration-300 text-secondary2 hover:text-black">
                                                    Ver
                                                </div>
                                        ...
                                        <div className="dots bottom-dot absolute bottom-[10%] left-[70%] cursor-pointer" {...storyblokEditable(dot)}
                                        >
                                            <div className="w-8 h-8 rounded-full bg-outline flex items-center justify-center"
                                                style={{ backgroundColor: dot.color_punto || '#ccc' }}
                                            >
