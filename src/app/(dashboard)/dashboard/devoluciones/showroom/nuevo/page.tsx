...
const NuevoCambio = () => {
  const { pedidosFacturados, loading, fetchPedido } = usePedidosFacturados("SHOWROOM")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.success) {
        toast.error(response.message || "Error al registrar el Showroom. Intenta nuevamente.");
        setLoadingSave(false)
        return;
      }
      else {
        toast.success("Showroom se registró correctamente.");
        setLoadingSave(false)
        fetchPedido()
        setSelectedReason("");
        setDetails("");
      ...
      console.log(error);
      setLoadingSave(false)
      toast.error("Error al registrar el showrrom. Intenta nuevamente.");
    }
  };

  const handleRegistrarCambio = async (item: DatosDev) => {
    try {
      const response = await getListMotivosService("SHOWROOM");
  ...
  return (
    <>
      ...
      <div className="flex bg-primary_sokso rounded-lg text-white sm:text-xs justify-center items-center  px-4 py-2 mt-4">
        ...
        <p className="text-sm">Registra aquí los productos  <a className="font-bold text-sm">NUEVOS SIN USO para SHOWROOM</a></p>
      ...
      {loading ? (
        ...
        <div className="text-center mt-24 text-sm text-gray-500 ">
          No se encuentraron pedidos para realizar el registro de Showroom.
        </div>
      ) : (
      ...
      >
        <ModalContent className="mx-8">
          <ModalHeader>Registrar Showroom</ModalHeader>
          <ModalBody>
