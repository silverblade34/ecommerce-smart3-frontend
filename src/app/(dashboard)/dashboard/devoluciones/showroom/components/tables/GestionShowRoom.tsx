...
export default function GestionShowRoom() {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const [selectedReason, setSelectedReason] = useState("");
  const { devolution, status, registrarBaseDevolucion } = useDevolution({ sTipoDevolucao: "SHOWROOM" });
  const { devoluciones, loading, confirmDelete } = useDevoluciones("SHOWROOM");
  ...
  const reasons = [
    ...
    {
      value: "No lo trajeron",
      label: "No lo trajeron",
      textValue: "No lo trajeron",
    },
  ...
  const confirmDelete2 = async () => {
    ...
    try {
      ...
      const result = await confirmDelete(productoToDelete, selectedReason);
      if (result) {
  ...
  return (
    <div className="pb-2">
      ...
      {devolution.sNumeroDevolucion != '' && !devolution.cardAviso?.isActive ?
        <Link
          href="/dashboard/devoluciones/showroom/nuevo"
          className="cursor-pointer rounded-lg  bg-primary_sokso p-2 mt-4 text-white "
        >
          <span className="text-xs"> + Nuevo ShowRoom</span>
        </Link>
        : null}
      ...
      {!devolution.cardAviso?.isActive && (
        <div className="my-2">
          {loading ? (
                  ...
                  title={
                    <div className="rounded-lg border-2 border-primary_sokso p-4 bg-[#e9d7f2] text-primary_sokso font-bold">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        <div className="w-full text-center sm:w-auto sm:text-left  mb-2">
                          <p className="font-bold text-xs">{cliente.sFullName}</p>
                          <p className="font-bold text-xs">{cliente?.detalles[0]?.sNumDocumento}</p>
                        </div>
                        <div className="flex justify-between w-full sm:w-auto sm:gap-8 sm:block">
                          <div className="text-center sm:text-left hidden sm:block text-xs">
                            s/. {cliente.totalMonto}
                          </div>
                        </div>
                        <div className="flex justify-between w-full sm:w-auto sm:gap-4">
                          <div className="text-center sm:text-left block sm:hidden px-4 text-xs">
                            s/. {cliente.totalMonto}
                          </div>
                          <div className="text-center sm:text-left px-4 text-xs">
                            {cliente.cantidadDevoluciones} / {cliente.cantidadAceptada}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <Card key={i} className="mb-4 p-2 mx-2">
                      <CardHeader className="flex justify-between items-start gap-x-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">Origen: {cliente.origen}</p>
                        </div>
                        <Button
                          isIconOnly
                          variant="flat"
                      ...
                      <CardBody>
                        ...
                        <div>
                          <p className="text-xs text-gray-500 mt-1.5">
                            F. Registro:{" "}
                            {dayjs(producto.fechaRegistroDevolucion)
            ...
            <div className="flex justify-center items-center h-32">
              <p className="text-[#8d8b8b]">
                No se encontraron registros de Devoluciones por Showroom.
              </p>
            </div>
          )}
        </div>
      ...
      >
        <ModalContent className="m-8">
          <div>
            <ModalHeader>
              <p className="text-sm">Motivo de Eliminación</p>
            </ModalHeader>
            <ModalBody>
              <Select
                size="sm"
