...
const NuevoCambio = () => {
  const { pedidosFacturados, loading, fetchPedido } = usePedidosFacturados("CAMBIO")
  ...
  const handleSearch = useCallback(() => {
    ...
    const results = pedidosFacturados?.filter((item: { sDatosEstrella: { sNombre: string; sApellidos: string; sNumeroDocumento: string; }; sDatosProducto: { sModelo: string; }; }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedReason === '2') {
      if (details === "") return toast.warning("Ingrese el detalle de la devolución");
      if (images.length < 3) return toast.warning("Debe adjuntar al menos 3 fotos.");
    }
    ...
    const payload = {
      ...
      sComentariosEvaluacion: selectedReason == '2' ? details : "",
      ...
      // sFotos: selectedReason == '2' ? images : [],
      ...(selectedReason === '2' && { sFotos: images })
    };
    try {
      setLoadingSave(true)
      const response = await postRegistrarCambioService(payload as unknown as Record<string, string>);
      if (!response.success) {
        toast.error(response.message || "Error al registrar el cambio. Intenta nuevamente.");
      ...
      console.log(error);
      setLoadingSave(false)
      toast.error("Error al registrar el cambio. Intenta nuevamente.");
    }
  };

  const handleRegistrarCambio = async (item: DatosDev) => {
    try {
      const response = await getListMotivosService("CAMBIO");
  ...
  const motivoSeleccionado = motivos.find(m => m.sIdMotivoDevolucion === selectedReason);
  const esProductoNoConforme = motivoSeleccionado?.sNombre.toLowerCase() === "producto no conforme";
  ...
  return (
    <>
        ...
        <div className="w-full sm:w-1/2">
          <Select
            ...
            onChange={(e) => {
              ...
              setSearchValue(""); // Limpiar el valor de búsqueda al cambiar el tipo
            }}
            size="sm"
            color="secondary"
            variant="flat"
      ...
      <div className="flex bg-primary_sokso rounded-lg text-white sm:text-xs justify-center items-center  px-4 py-2 mt-4">
        ...
        <p className="text-sm">Registra aquí los productos  <a className="font-bold text-sm">NUEVOS SIN USO para CAMBIO</a></p>
      ...
      {loading ? (
        ...
        <div className="text-center mt-24 text-sm text-gray-500 ">
          No se encuentraron pedidos para realizar el registro de CAMBIO.
        </div>
      ) : (
      ...
      >
        <ModalContent className="mx-8">
          <ModalHeader>Registrar Cambio</ModalHeader>
          <ModalBody>
            <div className="space-y-2 ">
              ...
              {/* <div className="flex justify-center">
                ...
                <div className="flex items-center space-x-2">
                  <Button onPress={decrementQuantity} size="sm">-</Button>
                  <p>{quantity}</p>
                  <Button onPress={incrementQuantity} size="sm">+</Button>
                </div>
                {selectedItem && (
                  <p className="text-xs text-gray-500 m-1">
                    Máximo permitido: {selectedItem.nCantidad}
                  </p>
                )}
              </div> */}
