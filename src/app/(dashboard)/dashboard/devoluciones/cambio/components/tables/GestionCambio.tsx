...
export default function GestionCambio() {
  ...
  const { devolution, status, registrarBaseDevolucion } = useDevolution({ sTipoDevolucao: "CAMBIO" });
  const { devoluciones, loading, confirmDelete } = useDevoluciones("CAMBIO");
  ...
  return (
    <div className="pb-2">
      ...
      {devolution.sNumeroDevolucion != '' && !devolution.cardAviso?.isActive ?
        <Link
          href="/dashboard/devoluciones/cambio/nuevo"
          className="cursor-pointer rounded-lg  bg-primary_sokso p-2 mt-4 text-white "
        >
          <span className="text-xs"> + Nuevo Cambio</span>
        </Link>
        : null}
          ...
          {loading ? (
            ...
            <div className="flex justify-center items-center h-32">
              <p className="text-[#8d8b8b]">
                No se encontraron registros de Devoluciones por CAMBIO.
              </p>
            </div>
          )}
        </div>
