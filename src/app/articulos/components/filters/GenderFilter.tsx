...
const GenderFilter = ({
  ...
  const handleGenderChange = (generoId: number, nomGenero:string) => {
    if (filtrosActuales.nIdGenero === generoId) {
      // Si ya está seleccionado, lo deseleccionamos
      setFiltros({ nIdGenero: undefined , nomGenero:undefined});
    } else {
      // Si no está seleccionado, lo seleccionamos
      setFiltros({ nIdGenero: generoId, nomGenero: nomGenero });
    }
  };
    ...
    >
      <h3 className="-my-3 flow-root">
        <DisclosureButton className="text-gray-400 hover:text-gray-500 group flex w-full items-center justify-between bg-white py-3 text-sm">
          <span className="heading6">Géneros</span>
      ...
      <DisclosurePanel className="list-color mt-4 flex max-h-32 flex-wrap items-center gap-3 gap-y-4 overflow-auto">
        {isLoading
          ...
          : generos?.map((genero) => {
              const isActive = filtrosActuales.nIdGenero === genero.nIdGenero;
              return (
                <button
                  key={genero.nIdGenero}
                  ...
                  onClick={() => handleGenderChange(genero.nIdGenero, genero.sDescripcion)}
                >
